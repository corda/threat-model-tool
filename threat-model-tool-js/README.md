...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build MkDocs site from threat models
        run: >
...
- **PDF/headless Chrome**: when `--generatePDF` is enabled, PDF generation uses Puppeteer via Docker (`ghcr.io/puppeteer/puppeteer:latest`) so you do not need to install Chrome manually in the runner.
- **Runner requirement**: Docker must be available in the GitHub runner (it is available on GitHub-hosted `ubuntu-latest`).
- **If Docker is not allowed** (self-hosted runner): install Java + PlantUML + Graphviz for diagrams, and Chromium/Chrome dependencies for PDFs, or disable `--generatePDF`.
