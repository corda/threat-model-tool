...
"${threat._id}" [ fillcolor="#F8CECC", style=filled, shape=polygon, color="#B85450"
    label= 
    <<table border="0" cellborder="0" cellspacing="0" width="520">
    ...
    <<table border="0" cellborder="0" cellspacing="0" width="xxx525">
...
"${tmo._id}" [fillcolor="#bae9ff", style=filled shape=ellipse, color="#2bbcff", label="${tmo._id}", label= 
    <<table border="0" cellborder="0" cellspacing="0">
     <tr><td align="center"><b>${tmo._id}</b><br/>${lib.wrapText(tmo.description)}</td></tr>
   </table>>]
...
% for threat in tmo.threats:
     ...
     "${threat._id}" -> "${tmo._id}" [label = " impacts"]
     ##      % endif     
     ## % endfor
% endfor
